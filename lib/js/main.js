...
var sweep = (function() {
  ...
  function tick () {
    ...
    batch.forEach(function (animation) {
      ...
      console.log(typeof animation.frame, animation.frame);
      if (frame === animation.end) {
        // remove the animation from batch and call its callback
        return batch.splice(array.indexOf(animation), 1)[0].callback();
      }
    });

    if (batch.length) requestAnimationFrame(tick);
  }

  function queueAnimation (animation) {
    if (batch.push(animation) === 1) requestAnimationFrame(tick);
  }
